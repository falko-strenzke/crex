...
- **Steps**:
  ...
  3. `TreeAction`: `MarkUp, MarkDown, Delete, Cut, Copy, PasteAfter, PasteBefore, PasteAsChild`. There is deliberately **no** separate `Yank` variant — vim's `y` and normal's Ctrl+C both mean `Copy` (see T002 step 5); the clipboard-vs-buffer-only distinction is `App`'s job in WP04, keyed on `App.bindings`, not the keymap's. Add a comment above the enum listing which existing keys (`i`, `I`, `e`, `E`, `J`, `K`, `z`, `/`) are deliberately **not** actions here — they stay hard-coded in `handle_document_key` because they are unchanged by this mission (FR-033) and adding them to this enum would be unnecessary churn.
